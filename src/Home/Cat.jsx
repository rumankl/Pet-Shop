...
const Cat = () => {
  ...
  // Filter foods to show only "Cat" category
  const CatFoods = data?.foods.filter((food) => food.category === "Cat");

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
      {CatFoods.length > 0 ? (
        CatFoods.map(({ _id, name, price, description, image }) => (
        ...
        <p className="text-center text-lg col-span-full">No Cat food available.</p>
      )}
    </div>
  );
};
