...
const Dog = () => {
  ...
  // Filter foods to show only "Dog" category
  const dogFoods = data?.foods.filter((food) => food.category === "Dog");

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
      {dogFoods.length > 0 ? (
        dogFoods.map(({ _id, name, price, description, image }) => (
        ...
        <p className="text-center text-lg col-span-full">No Dog food available.</p>
      )}
    </div>
  );
};
